...
def registrar_log(acao: str, detalhes: str = ""):
    """Registra ações do usuário para debug"""
    try:
        log_dir = APP_DIR / "logs"
        log_dir.mkdir(exist_ok=True)
        
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_file = log_dir / f"dsi_{datetime.date.today()}.log"
...
def buscar_si_duplo(service, d_ini_s: datetime.date, d_fim_s: datetime.date, 
                    d_ini_s1: datetime.date, d_fim_s1: datetime.date):
    """Busca SI para duas semanas (S e S+1) e retorna no formato "SI1/SI2" ou "-2/-1" se não encontrar"""
    ...
    if si_s and si_s1:
        ...
        return "-2/-1"  # Padrão quando não encontra nenhuma

def buscar_fase(service, d_ini_s: datetime.date, d_fim_s1: datetime.date):
    """Busca FASE que cobre o período completo (S + S+1)"""
...
def buscar_operacoes(service, d_ini_s: datetime.date, d_fim_s1: datetime.date):
    """
    Busca operações ativas no período (S + S+1) da agenda OPERAÇÕES.
    Retorna lista formatada: ["1) Op BR 135 (EMPREGO) - 08 Militares - 3º Sgt DANILO", ...]
    """
    ...
    for i, op in enumerate(operacoes_ordenadas, 1):
        if op['tipo']:
            # Deixar espaços para preenchimento manual de militares e responsável
...
def bullets_periodo(service, calendar_id: str, d_ini: datetime.date, d_fim: datetime.date, incluir_responsavel: bool = False):
    """Busca eventos que estejam ativos durante o período"""
    ...
    for ev in evs:
        ...
        if evento_ativo:
            ...
            if s:
                ...
                # Adicionar espaço para responsável se solicitado (para cursos)
...
def buscar_feriados(service, d_ini: datetime.date, d_fim: datetime.date):
    """Retorna conjunto de datas que são feriados"""
...
def construir_tabela_semana(service, d_ini: datetime.date, d_fim: datetime.date, incluir_cmt: bool, incluir_pgi: bool, feriados: set):
...
def exportar_excel(rows_s, rows_s1, num_fmt, si, fase, operacoes_linhas):
    """Exporta tabelas para Excel como backup"""
    try:
        output = io.BytesIO()
        ...
        registrar_log("AVISO", "openpyxl não instalado, usando CSV")
...
def salvar_historico(num_dsi: int, periodo: str, doc_id: str):
    """Salva histórico de DSIs geradas"""
    try:
        historico_file = APP_DIR / "historico.json"
        ...
        historico_file.write_text(json.dumps(historico, indent=2, ensure_ascii=False), encoding="utf-8")
...
def criar_google_doc(creds, titulo_doc, num_fmt, ref_date, ini_s, fim_s, ini_s1, fim_s1, 
    ...
    """Cria um Google Doc formatado conforme o modelo da DSI"""
    ...
    # 4. FORMATURA GERAL (NOVO - campos para preenchimento manual)
    ...
    docs_service.documents().batchUpdate(
        documentId=doc_id,
        body={'requests': requests_texto_s1}
    ).execute()
    ...
    # 6. ATIVIDADES FUTURAS (renumerado - com campos para preenchimento manual)
    conteudo_futuras = []
    ...
    # 7. SU (NOVO - com campos para preenchimento manual)
    ...
    docs_service.documents().batchUpdate(
        documentId=doc_id,
        body={'requests': requests_secao6_7}
    ).execute()
...
def inserir_e_preencher_tabela(docs_service, doc_id, rows, insert_index):
    """Insere uma tabela e preenche com os dados com mesclagem e cores alternadas"""
    requests_tabela = [{
        'insertTable': {
            'rows': len(rows) + 1,
            'columns': 7,
...
def aplicar_formatacao_tabela(docs_service, doc_id, rows, grupos_data):
    """Aplica mesclagem de células e cores alternadas"""
    doc = docs_service.documents().get(documentId=doc_id).execute()
    content = doc.get('body').get('content')
    ...
    # Aplicar bordas nas células da tabela
    ...
    # Formatar cabeçalho com fundo cinza escuro
    for col_idx in range(7):
        requests.append({
            'updateTableCellStyle': {
                'tableRange': {
    ...
    # Mesclagem de células da coluna DATA
    ...
    # Aplicar cores usando tableRange
    cor_alternada = True
    for data, indices in grupos_data.items():
    ...
    # Centralizar texto em TODAS as células da tabela
    ...
    if tabela_element:
        ...
        if requests_negrito:
            try:
                ...
                print(f"Erro ao aplicar negrito no cabeçalho: {e}")


def formatar_documento_completo(docs_service, doc_id, rows_s, rows_s1):
    """Aplica toda a formatação ao documento incluindo Calibri 12 e fonte vermelha"""
    time.sleep(1)
    ...
    # Padrões para negrito (ATUALIZADO com nova numeração)
    padroes_negrito = [
        r"1\.\s+OPERAÇÕES:",
    ...
    match_assinatura = re.search(r'JOÃO CARLOS DUQUE', texto_completo)
    if match_assinatura:
        linhas_antes = texto_completo[:match_assinatura.start()].split('\n')
        pos_sao_luis = len('\n'.join(linhas_antes[:-2])) + 1
        pos_fim_assinatura = match_assinatura.end() + len('\nComandante do 24º Batalhão de Infantaria de Selva')
        
        requests.append({
            'updateParagraphStyle': {
                'range': {
                    'startIndex': pos_sao_luis,
                    'endIndex': pos_fim_assinatura
                },
                'paragraphStyle': {
                    'alignment': 'CENTER'
                },
...
def criar_google_doc_safe(creds, *args, **kwargs):
    """Wrapper com tratamento robusto de erros"""
    max_tentativas = 3
...
try:
    ...
    with st.expander("🔍 Debug - SI, FASE e OPERAÇÕES"):
        st.write(f"**Período S (semana atual):** {ini_s.strftime('%d/%m/%Y')} a {fim_s.strftime('%d/%m/%Y')}")
        st.write(f"**Período S+1 (próxima semana):** {ini_s1.strftime('%d/%m/%Y')} a {fim_s1.strftime('%d/%m/%Y')}")
        st.write(f"**SI encontrado:** {si}")
        st.write(f"**FASE encontrada:** {fase}")
        st.write(f"**OPERAÇÕES encontradas:** {len(operacoes_linhas)}")
        for op in operacoes_linhas:
            st.write(f"  - {op}")

    # Buscar cursos com responsáveis
    ...
    # Processar exportação se solicitada
    ...
    # Mostrar mensagem de sucesso se documento foi criado
    ...
    try:
        ...
        registrar_log("ERRO_EXPORTACAO_BACKUP", str(e))
    ...
    df_s_display = pd.DataFrame(rows_s)
    df_s_display = df_s_display.drop(columns=['_especial'], errors='ignore')
    ...
    st.dataframe(
        df_s_display.style.apply(highlight_especial, axis=1),
        height=300
    )

    st.markdown(f"** b. 2. Semana (S+1) - {fmt_periodo_titulo(ini_s1, fim_s1)}**")
    
    df_s1_display = pd.DataFrame(rows_s1)
    df_s1_display = df_s1_display.drop(columns=['_especial'], errors='ignore')
    ...
    st.dataframe(
        df_s1_display.style.apply(highlight_especial_s1, axis=1),
        height=300
    )
